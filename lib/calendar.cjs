...
function calendar(data){
 ...
 let count=0;
 for(let day=Date.parse(start);day<=Date.parse(end);day+=86400000){const d=new Date(day),iso=d.toISOString().slice(0,10),weekday=(d.getUTCDay()+6)%7;if(excluded.has(iso))continue;
  for(const original of blocks.filter(b=>b.days.includes(weekday)&&(b.kind==='personal'||!data.schedule.off?.includes(weekday)))){const b={...original,...original.dayTimes?.[weekday]};
