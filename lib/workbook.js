...
export const WORKBOOK_VERSION='2026-09-23-flexible-touches';
...
export function validatePlan(data){
 ...
 requireThat(Array.isArray(touch.plan.slots)&&touch.plan.slots.length>=30,'Review your complete touch calendar.');
 ...
 for(const b of active){requireThat(typeof b.name==='string'&&b.name.length<=100&&b.days?.length&&b.days.every(d=>Number.isInteger(d)&&d>=0&&d<=6),'Review your weekly routine blocks.');let a=minute(b.start),z=minute(b.end);requireThat(a>=0&&z<=1440&&z>a&&a%15===0&&z%15===0,'Resolve invalid times in your routine.');for(const d of b.days){requireThat(b.kind==='personal'||!schedule.off?.includes(d),'Resolve routine blocks on a day off.');for(const o of active)if(o!==b&&o.days.includes(d))requireThat(!(a<minute(o.end)&&minute(o.start)<z),'Resolve overlapping routine blocks before sending.');}}
...
export async function workbook(raw){
 ...
 newPage('04 / Database Touch Plan');heading('Your '+t.slots.length+'-Touch Relationship Plan');paragraph(t.mets+' met contacts | Starts '+t.start);paragraph('Four personal calls to every met each year, using the DTD2 rotation. Honor each contact\'s communication preferences.');table(['DTD2 group',...t.quarters.map((q,i)=>'Quarter '+(i+1))],t.quarters[0].groups.map((group,j)=>[j===12?'Flex: Y / Z, other names and follow-up':group.letters,...t.quarters.map(q=>q.groups[j].date)]),[156,90,90,90,90]);
 newPage('04 / Touch Calendar');heading('Your relationship touch calendar');paragraph(Object.entries(t.channels||{}).filter(([,count])=>count).map(([channel,count])=>channel+': '+count).join(' | '),10);paragraph('Holiday cards go only to contacts for whom they are welcome. Birthday and anniversary entries are planning windows; use each person’s actual date. Verify local statistics and event details before sending.',10);
