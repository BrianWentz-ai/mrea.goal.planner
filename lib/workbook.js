...
export const WORKBOOK_VERSION='2026-09-21-compact-v2';
...
export function validatePlan(data){
 ...
 requireThat(schedule?.savedAt&&schedule.sourceKey===gps.saved.signature&&Array.isArray(schedule.blocks)&&schedule.blocks.length<=60,'Save your current weekly routine before sending.');
...
export async function workbook(raw){
 ...
 ['Listings / signed agreements',n(p.sellerSigned+p.buyerSigned),p.sellerAgreementsMonthly+p.buyerAgreementsMonthly],
 ['Contracts written',n(p.sellerContracts+p.buyerContracts),p.sellerContractsMonthly+p.buyerContractsMonthly],
 ['Contracts closed',n(p.sales),E.up(p.sellerSales/12)+E.up(p.buyerSales/12)],
 ...
 newPage('08 / Monthly Four Conversations');heading('Know your goal. See your progress.');paragraph('Month: __________________________');const rows=[['Appointments',p.monthly],['Listings + buyer commitments',p.sellerAgreementsMonthly+p.buyerAgreementsMonthly],['Closed units',E.up(p.sellerSales/12)+E.up(p.buyerSales/12)],['GCI',money(p.gci/12)]];const xx=[48,258,328,408,494];for(let i=0;i<5;i++)draw(['Monthly result','Goal','Actual','% of goal','GAP'][i],xx[i],y,10,bold,green);y+=32;for(const r of rows){draw(r[0],48,y,10);draw(String(r[1]),258,y,10,bold);for(let i=2;i<5;i++)draw('________',xx[i],y,10,normal,muted);y+= sixty();}paragraph('Percent of Goal = Actual / Goal x 100. GAP = Goal - Actual. A negative gap means ahead of goal. For a zero goal, write N/A.',10,normal,muted);paragraph('What will I adjust next month?',18,title);row('','________________________________');
