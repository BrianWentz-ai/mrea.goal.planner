...
const home=!!document.getElementById('annualTotal');
...
section.innerHTML=`<div class="contact-heading"><div><div class="contact-kicker">YOUR PERSONAL PLAN</div><h2>Make this plan yours</h2></div><button type="button" id="contactEdit">Edit contact details</button></div><p id="contactSummary"></p><form id="contactForm"><p>Personalize your plan and choose where we’ll email your completed workbook.</p><div class="contact-fields"><label>Full name <span>(required)</span><input name="name" autocomplete="name" maxlength="100" required></label><label>Email <span>(required)</span><input name="email" type="email" autocomplete="email" maxlength="254" required></label><label>Phone <span>(optional)</span><input name="phone" type="tel" autocomplete="tel" maxlength="40"></label></div><fieldset><legend>Optional updates from The Ascending Agent</legend><label class="contact-choice"><input type="checkbox" name="emailMarketing"> I’d also like marketing emails from The Ascending Agent. I can unsubscribe at any time.</label><label class="contact-choice"><input type="checkbox" name="textMarketing"> I’d also like marketing texts from The Ascending Agent at the number above. Message frequency varies; message and data rates may apply. Reply STOP to opt out.</label><p class="contact-small">These choices are optional and are not required to receive your plan. Selecting them does not enroll you or send messages yet.</p></fieldset><p class="contact-small">Your details stay in this browser unless you request a PDF or email delivery. This does not create an account. Backups may contain your contact details.</p><p id="contactError" role="alert"></p><div class="contact-buttons"><button type="submit" class="contact-primary" id="contactSave">Start My Plan</button><button type="button" id="contactCancel">Cancel</button></div></form>`;
...
function open(required=false){section.hidden=false;for(const name of ['name','email','phone'])form.elements[name].value=value[name];for(const name of ['emailMarketing','textMarketing'])form.elements[name].checked=value[name];form.hidden=false;summary.hidden=true;edit.hidden=true;cancel.hidden=required;error.textContent='';document.getElementById('contactSave').textContent=required?'Start My Plan':'Save Contact Details';if(required)document.body.classList.add('contact-required');}
function close(){section.hidden=true;form.hidden=true;summary.hidden=false;edit.hidden=false;document.body.classList.remove('contact-required');refresh();}
