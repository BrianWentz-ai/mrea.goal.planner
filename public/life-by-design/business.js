...
function restore(raw){
 ...
 if(raw.referredClosings===undefined){try{localStorage.setItem(KEY+':before-referral-count',JSON.stringify(raw));}catch{}v.referredClosings=1;v.referralFeePercent=25;v.saved=null;}
 delete v.budgets.sellerReferrals;delete v.budgets.buyerReferrals;
...
function control(key,label,help,min,max,step=1,unit='%',value=s[key],slider=true){
 ...
 return '<div class="control"><div class="control-top"><label for="n-'+key+'">'+esc(label)+(help?'<small>'+esc(help)+'</small>':'')+'</label><div class="value-field">'+(unit==='$'?'<span aria-hidden="true">$</span>':'')+'<input id="n-'+key+'" type="number" inputmode="decimal" min="'+min+'" max="'+(unit==='$'?100000000:unit==='weeks'?52:unit==='closings'?10000:100)+'" step="any" data-key="'+key+'" value="'+current+'" aria-label="'+esc(label)+'">'+(unit==='$'?'':'<span>'+esc(unit)+'</span>')+'</div></div>'+(slider?'<input type="range" aria-label="'+esc(label)+' slider" min="'+min+'" max="'+top+'" step="'+step+'" value="'+current+'" data-key="'+key+'"><div class="range-ends"><span>'+(unit==='$'?money(min):min+(unit==='%'?'%':''))+'</span><span>'+(unit==='$'?money(top):top+(unit==='%'?'%':''))+'</span></div>':'')+'</div>';
}
...
function referralMarkup(){
 return '<p class="help">Budget for referred closings using your buyer/seller mix and average GCI per closing.</p><div class="input-columns">'+control('referredClosings','Referred Closings','Number of closings with a referral fee this year.',0,50,1,'closings')+control('referralFeePercent','Referral Fee %','Percentage paid on each referred closing.',0,100,1,'%')+'</div><p id="referralCalculation" class="help" role="status"></p>';
}
...
function costPage(){
 ...
 accordion('referrals','Referral Fees',referralMarkup(),'Annual referred closings and referral fee.')+
...
function numbers(){
 ...
 if($('referralCalculation')){const average=s.sellerCommission*s.sellerShare/100+s.buyerCommission*(1-s.sellerShare/100);$('referralCalculation').textContent=s.referredClosings+' referred closings × '+money(average)+' weighted average GCI × '+s.referralFeePercent+'% = '+money(s.referredClosings*average*s.referralFeePercent/100)+' annual referral expense, included in Cost of Sales.';}
...
document.addEventListener('click',e=>{
 ...
 switch(b.dataset.action){
  ...
  case'mrea-conversions':Object.assign(s,{sellerSign:75,sellerClose:70,sellerContractClose:90,buyerSign:70,buyerClose:70,buyerContractClose:90});persist();render();break;
  case'new-conversions':Object.assign(s,{sellerSign:50,sellerClose:50,sellerContractClose:90,buyerSign:50,buyerClose:50,buyerContractClose:90});persist();render();break;
